...
int main(int argc, char* argv[])
{
    ...
    //codeletAvailable = (volatile unsigned *) (((char *)INTERFACE_COD_AVAIL_PTR) + cu_id * 0x4cU);
    //toFire = (volatile runt_codelet_t *) (((char *)INTERFACE_ACTIVE_COD_PTR) + cu_id * 0x4cU);
    codeletAvailable = CODELET_AVAILABLE(cu_id);
    toFire = TO_FIRE(cu_id);
    while(alive_sig) {
        if (*codeletAvailable) {
